...
export function StatsDashboard({ rows }: StatsDashboardProps) {
  ...
  const macroRings = [
    { label: "Protein", grams: averages.protein, percent: (macroCalories.protein / totalMacroCalories) * 100, color: "#2563eb" },
    { label: "Fat", grams: averages.fat, percent: (macroCalories.fat / totalMacroCalories) * 100, color: "#059669" },
    { label: "Carbs", grams: averages.carbs, percent: (macroCalories.carbs / totalMacroCalories) * 100, color: "#f97316" }
  ];
...
function MacroRings({ macros }: { macros: Array<{ label: string; grams: number; percent: number; color: string }> }) {
  return (
    <section className="animate-enter-soft rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <h2 className="text-lg font-semibold">Macro split</h2>
      <p className="mt-1 text-sm text-slate-500">Average macro calories in this range.</p>
      <div className="mt-5 grid gap-4">
        {macros.map((macro) => (
          <div key={macro.label} className="flex items-center gap-4 rounded-lg bg-slate-50 p-3">
            <ProgressRing color={macro.color} percent={macro.percent} />
            <div>
              <p className="font-semibold">{macro.label}</p>
              <p className="text-sm text-slate-500">{round(macro.grams)} g average</p>
            </div>
            <p className="ml-auto text-xl font-semibold" style={{ color: macro.color }}>
              {round(macro.percent)}%
            </p>
          </div>
        ))}
